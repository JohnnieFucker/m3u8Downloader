...
function download(opts, cb) {
    ...
    url = opts.url;
    if (opts.filmName) {
        filmName = opts.filmName.replace(/ /g, '_');
    ...
    if (tsList[0].hasOwnProperty('key') && tsList[0].key.hasOwnProperty('uri')) {
        ...
        if (tsList[0].key.uri.indexOf('http') < 0) {
            if (tsList[0].key.uri.indexOf('//') === 0) {
                key = `${urlObj.protocol}${tsList[0].key.uri}`;
            } else {
                if (tsList[0].key.uri.indexOf('/') === 0) {
...
function decryptTs(index, tsObj, needLog) {
    if (!fs.existsSync(`${tsObj.file}x`)) {
...
function downloadTs(index) {
    ...
    if (fs.existsSync(tsObj.file)) {
        utils.log(`ts${tsObj.index} exist,downloaded ${downloadedNum}/remain${tsCount - downloadedNum}`);
        if (key !== 'nokey' && method === 'AES-128') {
            decryptTs(index, tsObj);
        } else {
            downloadedNum++;
            checkIfDone();
            downloadTs(index + processNum);
